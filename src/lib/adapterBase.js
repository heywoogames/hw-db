...
class AdapterBase {
  ...
  updateDialectCfg(_cfgCur, _cfgNew) {
    this._mgr._plug.app.logger.warn(`xx adapter ${this._name} updateDialectCfg not implement`);
  }

  async testConn() {
    this._mgr._plug.app.logger.warn(`xx adapter ${this._name} testConn not implement`);
  }
   ...
   */
  async start() {
    this._mgr._plug.app.logger.warn(`xx adapter ${this._name} start not implement`);
  }
  ...
  async addRecord(_tbName, _data) {
    this._mgr._plug.app.logger.warn(`xx adapter ${this._name} addRecord not implement`);
   ...
   */
  validCfg(_cfg) {
    this._mgr._plug.app.logger.warn(`xx adapter ${this._name} validCfg not implement`);
  }
  ...
  setlogging(_dbInsName, _lv) {
    this._mgr._plug.app.logger.warn(`xx adapter ${this._name} setlogging not implement`);
  }
}
