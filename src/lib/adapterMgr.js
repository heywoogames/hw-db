...
class AdapterMgr {
  ...
  constructor(plug) {
    ...
    /** @type {Record<string, import('@types').AdapterBase>} */
    this._adapters = {};
    ...
    /** @type { Record<string, {dbKey: string, adaIns: import('@types').AdapterBase}>} db 和db 实例名映射*/
    this.dbNameMap = {};

    /** @type { Record<string, import('@types').ModelXItem>} 表和模型的映射关系*/
    this.tableModelMap = {};
    ...
    files.forEach((file) => {
      if (file.endsWith(".js")) {
        try {
          ...
          /** @type {import('@types').AdapterBase} */
          const ins = new adapter(this);
          this.registerAdapter(ins);
        } catch (/** @type {any} */ err) {
          this._plug.app.logger.warn(`-- load adapter ${file} error: ${err.message}`);
        }
      }
    });

    for (const dia in this._dialectMap) {
      this._plug.app.logger.info(`-- dialect ${dia} adapter ${this._dialectMap[dia]._name}`);
    }
  }
  ...
  /**
   * @param {Record<string, import('../../index').DialectCfg>} dbs - 数据库
   */
  initDBCfg(dbs) {
    if (!dbs) {
      return;
    ...
    for (const key in dbs) {
      ...
      /** @type { import('@types').AdapterBase} */
      const diaIns = this._dialectMap[dialect];
      if (diaIns === undefined) {
        this._plug.app.logger.warn(`-- dialect ${dialect} not support`);
        continue;
      }
  ...
  /**
   * 注册适配器
   * @param {import('@types').AdapterBase} adapter - 适配器实例
   */
   ...
   * @returns { import('@sequelize/core').Sequelize | null }
   */
  getDBIns(dbInsName) {
  ...
  /**
   * 获取所有模型
   * @returns {Object.<string, import('@types').ModelXItem>}
   */
  getModels() {
